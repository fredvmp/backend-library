...
import psycopg2
from services.metrics_service import get_reading_summary, get_genre_reading_velocity, get_genre_dropout_rate, get_genre_format_popularity, get_user_reading_velocity
...
@metrics_bp.route("/user-reading-velocity", methods=["GET"])
def user_reading_velocity():
    result = get_user_reading_velocity()
    if result.empty:
        return jsonify({"message": "No data available"}), 200
    return result.to_html(classes="table table-striped"), 200